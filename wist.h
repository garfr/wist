...
/* 
 * There are currently two types of wist handles, handles allocated on a 
 * handle stack, and persistent handles. 
 * 
 * All handles returned by normal wist functions are allocated on the current 
 * stack, and when the stack is popped are all freed.  These handles can be 
 * made persistent through wist_handle_freeze.
 */
...
/* Pushes a new frame to allocate handles on. */
void wist_handle_stack_push(struct wist_vm *vm);

/* Pops the current frame releasing all handles allocated on it. */
...
/* Parses and typechecks a declaration. */
struct wist_parse_result *wist_compiler_parse_decl(struct wist_compiler *comp,
        const uint8_t *src, size_t src_len, struct wist_ast_decl **decl);

/* 
 * Connects a compiler to a VM, must be done before calling any other 
 * wist_compiler_vm_* functions.
 . */
void wist_compiler_vm_connect(struct wist_compiler *comp, struct wist_vm *vm);

/* Compiles a closure that evaluates an expression. */
struct wist_handle *wist_compiler_vm_gen_expr(struct wist_compiler *comp,
        struct wist_vm *vm, struct wist_ast_expr *expr);

/* Compiles a closure that will execute a declaration. */
...
/* Releases an AST declaration generated by the compiler. */
